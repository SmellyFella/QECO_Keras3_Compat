...
class Heuristic_DQN:
    ...
    def choose_action(self, observation):
      ...
      # Extract fields from observation vector:
      task_size = observation[0]
      meter_compute_time = observation[1]
      meter_transmit_time = observation[2]
  
      # b_substation_comp is a vector – substation computational loads
      # we assume they start at index 3 and have length n_actions-1
      subs_loads = observation[3:3 + (self.n_actions - 1)]
  
      meter_energy_state = observation[3 + (self.n_actions - 1)]
      deadline_remaining = observation[4 + (self.n_actions - 1)]
      meter_local_queue_len = observation[5 + (self.n_actions - 1)]
      meter_transmit_queue_len = observation[6 + (self.n_actions - 1)]
      substation_queue_len = observation[7 + (self.n_actions - 1)]  # single scalar OR expand if per-substation later
  
      # Action 0 = local computation
      # Actions 1..N = send to substations
      local_compute_time = meter_compute_time + meter_local_queue_len
  
      # Estimated remote processing = transmission + substation queue + comp load
      remote_est = subs_loads + meter_transmit_time + substation_queue_len
  
      # BEST remote choice
      best_substation_idx = np.argmin(remote_est)
      best_remote_time = remote_est[best_substation_idx]
  
      # Decision rule:
      if local_compute_time <= best_remote_time:
          return 0  # Local compute is best
      else:
          return best_substation_idx + 1  # Offset by 1 (action 1 = substation 0)
